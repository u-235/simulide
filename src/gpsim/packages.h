...
public:
  ...
  uint isa(void){return _NO_PACKAGE_;};
  ...
  virtual float get_pin_position(uint pin_number);
  virtual void set_pin_position(uint pin_number, float position);
  int pin_existance(uint pin_number);
  IOPIN *get_pin(uint pin_number);

  void setPinGeometry(uint pin_number, float x, float y, int orientation, bool bShowName);
  PinGeometry *getPinGeometry(uint pin_number);

  // Debug
  void showPins();

protected:
  inline bool bIsValidPinNumber(uint pin_number)
  {
